...
- Nothing else. No runtime dependencies, no companion scripts, no files outside
  the plugin directory. It shells out to `hyprctl`, plus the POSIX `sh`, the
  coreutils `head`, and `systemd-run` — which runs each probe in a transient
  service bounded in output (`head -c`) and runtime (`RuntimeMaxSec`) under the
  systemd **user** manager (running on any Omarchy system) — so there is nothing
  extra to install.
